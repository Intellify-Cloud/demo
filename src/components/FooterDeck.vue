<template>
  <footer class="bg-surface-muted">
    <div
      class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 grid grid-cols-1 lg:grid-cols-[2fr_3fr] gap-12"
    >
      <div>
        <RouterLink
          to="/"
        ...
        <div v-for="column in site.footer.columns" :key="column.title">
          ...
          <ul class="space-y-3">
            <li v-for="link in column.links" :key="link.label">
              <component
                :is="isExternal(link.href) ? 'a' : 'RouterLink'"
    ...
    >
      <p class="text-sm">&copy; {{ currentYear }} {{ site.name }}. All rights reserved.</p>
